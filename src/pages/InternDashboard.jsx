...
const NavItem = ({ icon: Icon, label, sublabel, active, onClick, badge }) => (
  ...
  >
    <div className={`p-1.5 rounded-lg ${active ? 'bg-emerald-100' : 'bg-gray-100 group-hover:bg-gray-200'}`}>
...
const StatCard = ({ label, value, color }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-5">
      ...
      <p className={`text-3xl font-black ${colors[color] || 'text-gray-900'}`}>{value}</p>
    </div>
  );
};
